...
## 错误码
- `store.invalid_category` (400): 尝试将门店关联到不存在或已禁用的服务分类时，抛出此错误。
- `store.store_not_found` (404): 请求的门店不存在时，抛出此错误。
- `validation.invalid_input` (400): 预约规则校验失败（如非法的预约粒度、取消时限等）或表单输入非法时，抛出此错误。
- `router.not_found` (404): 请求的接口路径或方法不存在时，抛出此错误。
