...
## Error Codes
- `service.invalid_category` (400): category_id 指向不存在或已停用的全局分类。
- `service.category_not_in_store` (400): category_id 不在该门店 store_category 已声明的分类集合内。
- `service.service_not_found` (404): 请求的服务项目不存在(或不属于本店)。
- `validation.invalid_input` (400): 价格非整数/负数、时长缺失或非正、其他字段校验失败。
